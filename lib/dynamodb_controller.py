...
class DynamoDBController:
    ...
    def query_with_pagination(self, model_class: Type[Model], key_condition_expression: Any, filter_expression: Optional[Any] = None, index_name: Optional[str] = None, limit: int = 20, last_evaluated_key: Optional[Dict[str, Any]] = None) -> Tuple[List[Model], Optional[Dict[str, Any]]]:
        ...
        if hasattr(key_condition_expression, 'attribute') and hasattr(key_condition_expression, 'startswith_value'):
            response = model_class.query(
                hash_key=key_condition_expression.attribute,
                range_key_condition=model_class.SK.startswith(key_condition_expression.startswith_value),
                **query_params
            )
        else:
            response = model_class.query(
